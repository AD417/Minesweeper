...
class MineArray {
    ...
    isMine(x, y) { // Tell if a tile contains a mine. 
        return (x >= 0 && y >= 0 && x < this.rows && y < this.cols && this.tileData[x][y].isMine);
    }
    ...
    HTMLForTile(x, y) {
        if (gameHasEnded && this.isMine(x, y)) {
            return `<div 
            id="tile-${x}-${y}" 
            class="tile mine uncovered">
            X</div>`
        }
        if (this.isMine(x, y)) {
            return `<div 
            id="tile-${x}-${y}" 
            class="tile mine covered"
            onclick="endGame()">X
            </div>`
        }
        return `<div 
    ...
    displayHTML() { // Creates the tile that is rendered on the screen. 
